...
socket.on('your-turn', data => {
    ...
    // Disable alien button and let captain know it's their turn
    alienBtn.disabled = true
    ...
    // Last element of array is the number of players the captain must test
...
function checkOptionLimit(box) {
    ...
    // Reset our option counter
    numOptionsChosen = 0

    // Iterate through all the checkboxes and for each one ticked, increment the option chosen counter and make the box's id our optionChosen value
    ...
    if (numOptionsChosen === 1) {
        ...
        for (let y = 0; y < optionCheckboxes.length; y++) {
            ...
            // If the box hasn't been ticked then disable the box so that it can't be checked as we have reached the limit of 1
            optionCheckboxes[y].disabled = true
        }
    }

    // If the number of options chosen is less than the limit
    if (numOptionsChosen < 1) {
...
function createPlayerButtons(data) {
    // Empty the selection area

    // Create a button for each player sent in the choose-aliens message
    selection.innerHTML = ''
...
function checkPlayerLimit() {
    ...
    // Wipe our players chosen list
    playersChosen.splice(0, playersChosen.length)

    // Iterate through all the checkboxes and for each one ticked, increment the players chosen counter and add the box's id to our players chosen list
...
socket.on('choosing', data => {
    // Disable the alien button
    alienBtn.disabled = true

    // If the player themselves isn't the captain then display the name of the captain
...
socket.on('writing', data => {
    // Empty game rea then display prompt
    gameArea.innerHTML = ''
    ...
    // Add a <br> character for spacing
    ...
    // Add the submit button and assign onClick function
...
socket.on('drawing', data => {
    // Empty gameArea and display the prompt
    gameArea.innerHTML = ''
    let prompt = document.createElement('div')
    prompt.textContent = data
    prompt.style.textAlign = 'center'
...
socket.on('tier', data => {
    ...
    // Empty game area and add prompt
    gameArea.innerHTML = ''
    let prompt = document.createElement('div')
...
<div id="bank"></div>`
    ...
    // Helper functions for tier list
...
function sendTier() {
    // Scroll to the top so we can view the whole tier list
    gameArea.scrollTop = 0
    tierList = document.getElementById('tierListBoard')

    // Essentially screenshot the tier list and send as image
...
function resetAreas() {
    gameArea.innerHTML = "Completed the task."
    selection.innerHTML = ''
}
...
socket.on('trivia', data => {
    ...
    // Empty game area and display prompt
    gameArea.innerHTML = ''
